        ...
        if st.button("📨 이 브리핑을 내 이메일로 보내기", key="send_email_btn"):
            ...
            with st.spinner("이브가 브리핑을 전송 중입니다... 💌"):
                if send_email(d['ai_text'], d['news_text']):
                    st.success("✅ 메일 발송 성공!")
# (기존 가입 완료 / 로그인 성공 코드 아래 부분에 추가)
    
    st.divider()
    st.subheader("📢 이브의 실시간 속보 채널")
    st.write("앱에 들어오지 않아도 텔레그램으로 가장 먼저 주요 경제 속보를 받아보세요!")
    # 👇 링크 부분에 대표님이 아까 만드신 채널 주소(t.me/...)를 넣어주세요!
    st.link_button("📲 텔레그램 공식 채널 입장하기", "https://t.me/여기에_채널_링크_입력", type="primary", use_container_width=True)
